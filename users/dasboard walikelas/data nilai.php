...
<?php
  require_once('../../koneksi.php');
	if(isset($_POST['simpan'])){
		...
		$nis = $_POST['nis'];
		$mapel = $_POST['mapel'];
		$qry = "insert into nilai_ipa values ('','$nis','$mapel','','','$nilai')";
      ...
      <table id="example1" class="table table-bordered table-striped">
        ...
        <tbody>
					<?php
								...
								if (empty($cari)) {
									$qry = mysqli_query($conn, "SELECT * from data_siswa_ipa a, nilai_ipa b, mapel c WHERE a.nis=b.nis AND b.kode_mapel=c.kode_mapel");
								}
								else {
									$qry = mysqli_query($conn, "SELECT * from data_siswa_ipa a, nilai_ipa b, mapel c WHERE a.nis=b.nis AND b.kode_mapel=c.kode_mapel AND a.nis = '$cari'");
								}
								$no=1;
        ...
        <tr>
           <td><?php echo $no; ?></td>
						...
						<td><center>
							<a href="ubah_nilaiwalikelas.php?nis=<?php echo $data['nis']; ?>"><span data-toggle="tooltip" title="Edit Akun"><button class="btn btn-warning"><i class="fa fa-edit"></i></button>
								</a>
							<a href="hapus_nilaiwalikelas.php?nis=<?php echo $data['nis']; ?>" onclick="return confirm('Apakah anda yakin menghapus data ini?')">
