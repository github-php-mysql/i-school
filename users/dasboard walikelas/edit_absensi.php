...
if (empty($cari)) {
	$nisbaru = $_GET['nis'];
  $qry = mysqli_query($conn, "SELECT * from data_siswa_ipa where nis = '$nisbaru'");
} else {
...
<?php
	if(isset($_POST['simpan'])){
		...
		$query = "update kehadiran_ipa set kehadiran = '$mapel' where nis = '$nis'";
		$masuk = mysqli_query($conn, $query);
		//unset($_POST['simpan']);
		if ($query){
			echo "<script>alert('berhasil disimpan');document.location='data absensi.php'</script>";
		}
		else {
			echo 'gagal';
		}
