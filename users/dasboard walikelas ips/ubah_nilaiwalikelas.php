...
<?php
  require_once('../../koneksi.php');
	if(isset($_POST['simpan'])){
		...
		$nis = $_POST['nis'];
		$mapel = $_POST['mapel'];
		$qry = "update nilai_ips set uts = '$uts', harian = '$nilai', nilai_semester = '$smt' where nis = '$nis'";
...
$mapelTabel="<select name='mapel'>";
$resultMapel = mysqli_query($conn, "SELECT * FROM mapel");
while($mapel = mysqli_fetch_array($resultMapel)){
  $mapelTabel.="<option value=".$mapel['kode_mapel'].">".$mapel['nama_mapel']."</option>";
}
$mapelTabel.="</select>";
if (empty($cari)) {
  $qry = mysqli_query($conn, "SELECT * from data_siswa_ips,nilai_ips where nilai_ips.nis = data_siswa_ips.nis");
} else {
  $qry = mysqli_query($conn, "SELECT * from data_siswa_ips,nilai_ips where nilai_ips.nis = data_siswa_ips.nis AND data_siswa_ips.nis = '$cari'");
}
...
?>
        <tr>
          ...
          <td>
			<div class="col-xs-8">
				<input type="number" class="form-control" placeholder="Nilai" name="nilai" value="<?php echo $data['harian'] ?>">
			</div>
          </td>
		  <td>
			<div class="col-xs-8">
				<input type="number" class="form-control" placeholder="Nilai" name="uts" value="<?php echo $data['uts'] ?>">
			</div>
          </td>
		  <td>
			<div class="col-xs-8">
				<input type="number" class="form-control" placeholder="Nilai" name="semester" value="<?php echo $data['nilai_semester'] ?>">
			</div>
          </td>
          <td>
