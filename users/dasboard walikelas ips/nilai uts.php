...
<?php
  require_once('../../koneksi.php');
	if(isset($_POST['simpan'])){
		...
		$nis = $_POST['nis'];
		$mapel = $_POST['mapel'];
		$result = mysqli_query($conn, "SELECT * FROM nilai_ips WHERE nis='$nis'");
		$num_rows = mysqli_num_rows($result);
		//echo $num_rows;
		//die();
		if($num_rows>0){
			$qry = "update nilai_ips set nis='$nis', kode_mapel='$mapel', uts='$nilai' WHERE nis='$nis'";
			//echo $qry;
			//die();
		}else {
			$qry = "insert into nilai_ips values ('','$nis','$mapel','','$nilai','')";
		}
