...
if (empty($cari)) {
	$nisbaru = $_GET['nis'];
  $qry = mysqli_query($conn, "SELECT * from pembayaran_ips where nis = '$nisbaru'");
} else {
  $qry = mysqli_query($conn, "SELECT * from pembayaran_ips where nama like '%".$cari."%'");
}
...
<?php
	if(isset($_POST['simpan'])){
		...
		$query = "update pembayaran_ips set pembayaran = '$pembayaran', tanggal = '$tanggal' where nis = '$nis'";
		$masuk = mysqli_query($conn, $query);
		//unset($_POST['simpan']);
		if ($query){
			echo "<script>alert('berhasil disimpan');document.location='data pembayaran.php'</script>";
		}
		else {
			echo 'gagal';
		}
